<x-app-layout>
    ...
    <div class="py-12" x-data="itemForm()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('items.store') }}" method="POST" enctype="multipart/form-data" id="itemForm" class="flex flex-col lg:flex-row gap-8">
                @csrf
                ...
                <div class="flex-1 space-y-6">
                    <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="title" name="title" type="text" class="mt-1 block w-full" x-model="title" required placeholder="Misal: Sepeda Lipat Element, Buku Pemrograman Laravel..." />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="quantity" name="quantity" type="number" min="1" class="mt-1 block w-full" value="1" required />
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm" required placeholder="Ceritakan detail barang ini, riwayat penggunaan, minus (jika ada), dll..."></textarea>
                            ...
                            <div class="grid grid-cols-2 gap-3" x-model="condition">
                                <label class="border rounded-lg p-3 cursor-pointer hover:bg-teal-50 transition flex items-start gap-2" :class="condition === 'new' ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-gray-200'">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-xs text-gray-500">Belum pernah dipakai / masih segel.</span>
                                    </div>
                                </label>
                                <label class="border rounded-lg p-3 cursor-pointer hover:bg-teal-50 transition flex items-start gap-2" :class="condition === 'like_new' ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-gray-200'">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-xs text-gray-500">Pernah dipakai namun tanpa cacat.</span>
                                    </div>
                                </label>
                                <label class="border rounded-lg p-3 cursor-pointer hover:bg-teal-50 transition flex items-start gap-2" :class="condition === 'good' ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-gray-200'">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-xs text-gray-500">Sering dipakai, fungsi normal, sedikit minus fisik.</span>
                                    </div>
                                </label>
                                <label class="border rounded-lg p-3 cursor-pointer hover:bg-teal-50 transition flex items-start gap-2" :class="condition === 'fair' ? 'border-teal-500 bg-teal-50 ring-1 ring-teal-500' : 'border-gray-200'">
                                    ...
                                    <div>
                                        ...
                                        <span class="block text-xs text-gray-500">Banyak minus fisik, tapi masih berfungsi.</span>
                                    </div>
                                </label>
                            </div>
                    ...
                    <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                        ...
                        >
                            <input type="file" name="images[]" id="images" multiple accept="image/jpeg,image/png,image/jpg,image/webp" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" @change="handleFileSelect($event)" :disabled="images.length >= 5">
                            ...
                            <div class="flex flex-col items-center pointer-events-none">
                                ...
                                <p class="text-xs text-gray-500 mt-1">Format: JPG, PNG, WEBP. Maks: 5 Foto (2MB/foto).</p>
                                <p class="text-sm font-bold mt-3 text-teal-600" x-text="images.length + '/5 Foto Terpilih'"></p>
                            </div>
                        </div>
                    ...
                    <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="location" name="location" type="text" class="mt-1 block w-full" x-model="location" required placeholder="Misal: Bandung, Jawa Barat" />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="delivery_method" name="delivery_method" class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm" required>
                                <option value="pickup">Penerima Ambil Sendiri (Pickup)</option>
                                <option value="delivery">Saya Akan Mengantar (Delivery)</option>
                                <option value="both">Bisa Keduanya</option>
                            </select>
                            <x-input-error class="mt-2" :messages="$errors->get('delivery_method')" />
                        </div>

                        <!-- Event Opsional -->
                        @if($events->count() > 0)
                        <div class="mb-4">
                            <x-input-label for="event_id" value="Sertakan ke Event Donasi (Opsional)" />
                            <select id="event_id" name="event_id" class="mt-1 block w-full border-gray-300 focus:border-teal-500 focus:ring-teal-500 rounded-md shadow-sm">
                                <option value="">Tidak mengikuti event apapun</option>
                    ...
                    <div class="sticky top-20">
                        ...
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            <div class="aspect-w-16 aspect-h-9 relative bg-gray-200">
                                ...
                                <div class="absolute top-3 left-3 bg-white/90 backdrop-blur text-gray-800 text-xs px-2 py-1 rounded font-semibold shadow-sm" x-text="categoryName() || 'Kategori'"></div>
    ...
    <script>
        function itemForm() {
            return {
                ...
                processFiles(files) {
                    let remainingSlots = 5 - this.images.length;
                    ...
                    fileArray.forEach(file => {
                        if (file.type.startsWith('image/') && file.size <= 2 * 1024 * 1024) {
                            ...
                            alert('File ' + file.name + ' tidak didukung atau ukurannya lebih dari 2MB.');
                        }
                    });
                },
                ...
                syncFileInput() {
                    // Update hidden file input using DataTransfer API before submitting
