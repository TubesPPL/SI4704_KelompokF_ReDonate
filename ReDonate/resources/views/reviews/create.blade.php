        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                <div class="p-8 border-b border-gray-100 bg-white">
                    <h3 class="text-2xl font-extrabold text-gray-900 mb-2 text-center">Terima Kasih! 🎉</h3>
                    <p class="text-gray-500 text-center mb-8">Barang telah berhasil Anda terima. Bagaimana pengalaman Anda dengan donatur?</p>
                    
                    <div class="flex items-center justify-center gap-4 p-4 bg-teal-50 rounded-xl border border-teal-100 mb-8">
                        <img class="h-16 w-16 rounded-full object-cover shadow-sm border-2 border-white" src="{{ $claim->item->user->avatar ? Storage::url($claim->item->user->avatar) : 'https://ui-avatars.com/api/?name='.urlencode($claim->item->user->name).'&color=0D9488&background=CCFBF1' }}" alt="{{ $claim->item->user->name }}">
                        <div>
                            <p class="text-xs text-teal-600 font-semibold uppercase tracking-wider">Mendonasikan:</p>
                            <p class="font-bold text-gray-900 text-lg">{{ $claim->item->title }}</p>
                            <p class="text-sm text-gray-600 font-medium">Oleh: {{ $claim->item->user->name }}</p>
                        </div>
                    </div>

                    <form action="{{ route('reviews.store', $claim->id) }}" method="POST" x-data="reviewForm()">
                        @csrf
                        ...
                        <div class="mb-8">
                            ...
                            <textarea id="comment" name="comment" rows="4" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-teal-500 focus:ring-teal-500" placeholder="Ucapkan terima kasih atau berikan komentar atas pengalaman Anda..."></textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('comment')" />
                        </div>

                        <div class="flex justify-end gap-3">
                            <a href="{{ route('recipient.dashboard') }}" class="px-6 py-3 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition">Nanti Saja</a>
